-- Fase 6H: telemetria segura da inferência híbrida executada pela Edge Function.
...
begin
  if current_user <> 'service_role' then raise exception 'Acesso negado' using errcode='42501'; end if;
...
create or replace function public.record_menu_ai_staging_event(target_slug text,target_session_token text,target_message_hash text,target_route text,target_outcome text,target_provider text,target_model text,target_latency_ms integer,target_input_tokens integer,target_output_tokens integer,target_estimated_cost_micros integer)
returns bigint language sql security invoker set search_path='' as $$select private.record_menu_ai_staging_event(target_slug,target_session_token,target_message_hash,target_route,target_outcome,target_provider,target_model,target_latency_ms,target_input_tokens,target_output_tokens,target_estimated_cost_micros)$$;
revoke all on function public.record_menu_ai_staging_event(text,text,text,text,text,text,text,integer,integer,integer,integer) from public,anon,authenticated;
grant execute on function public.record_menu_ai_staging_event(text,text,text,text,text,text,text,integer,integer,integer,integer) to service_role;
