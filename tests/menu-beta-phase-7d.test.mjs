...
test("resultado humano aprova os quatro segmentos sem liberar produção", async () => {
  const result = await readJson("config/menu-beta-phase-7d-result.json");
  ...
  assert.ok(Object.values(result.segmentResults).every((segment) => segment.completed && segment.easeScore === 10 && segment.finalStatusVisible === "Recebido"));
});
